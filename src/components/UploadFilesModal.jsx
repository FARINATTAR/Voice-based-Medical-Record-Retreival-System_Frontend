...
const UploadFilesModal = ({ onClose, onSuccess }) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      setError('Failed to load patients and doctors. Please refresh.');
  ...
  const handleModalClose = () => {
    ...
    if (selectedFiles.length > 0) {
      if (window.confirm('You have unsaved files. Are you sure you want to close?')) {
        onClose();
      }
    } else {
      onClose();
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  ...
                  >
                    <option value="">
                      {patients.length === 0 ? 'No patients available' : 'Choose a patient'}
                    </option>
                    {patients.map((patient) => (
                      <option key={patient._id} value={patient._id}>
                        {patient.name} - {patient.age}y ({patient.phone})
                    ...
                    >
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <button
                          ...
                          onClick={() => removeFile(index)}
                          disabled={loading}
                          className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition disabled:opacity-50 disabled:cursor-not-allowed flex-shrink-0"
              ...
              <div className="flex space-x-4 pt-4">
                ...
                <button
                  type="submit"
                  disabled={loading || selectedFiles.length === 0 || loadingData}
                  className="flex-1 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-blue-300 disabled:cursor-not-allowed transition"
                >
                  {loading ? 'Uploading...' : `Upload ${selectedFiles.length} File(s)`}
                </button>
              </div>
